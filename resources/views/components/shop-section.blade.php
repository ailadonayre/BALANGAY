<section class="py-16 md:py-20 lg:py-24 bg-[#E4DDCC] reveal" id="shop">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @foreach($products as $index => $product)
                <div class="group cursor-pointer loading product-card shop-product-hero" data-product-id="{{ $product['id'] }}" style="animation-delay: {{ $index * 0.1 }}s">
                    ...
                    <div class="relative overflow-hidden rounded-lg bg-white aspect-square mb-4 shadow-md">
                        <img data-src="{{ asset('assets/products/' . $product['image']) }}" loading="lazy" decoding="async" alt="{{ $product['name'] }}" class="w-full h-full object-cover object-center transition-transform duration-700 group-hover:scale-110 lazy">
        ...
        <div class="text-center mt-16">
            <a href="/shop" class="inline-block bg-[#5B5843] text-white px-12 py-4 rounded-full text-sm tracking-widest uppercase futura-500 hover:bg-[#5B5843] transition-all duration-300 transform hover:scale-105">
                View All Products
            </a>
        </div>
    </div>
