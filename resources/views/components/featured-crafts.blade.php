<section class="py-24 bg-[#F8F4EE]" id="crafts">
    <div class="max-w-7xl mx-auto px-6 lg:px-8">
        <div class="text-center mb-16 loading">
            <h2 class="text-4xl md:text-5xl lg:text-6xl mb-6">Featured Crafts</h2>
        ...
        <div class="grid md:grid-cols-3 gap-8 lg:gap-12">
            ...
            <div class="group loading" style="animation-delay: {{ $index * 0.2 }}s">
                <div class="relative overflow-hidden rounded-2xl bg-white aspect-square mb-6 shadow-lg">
                    ...
                    <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center justify-center">
                        <button class="bg-white text-[#252525] px-8 py-3 rounded-full text-sm tracking-wider uppercase futura-500 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                            View Details
                        </button>
                    </div>
                </div>
                <div>
                    <h3 class="text-xl mb-2 futura-500">{{ $craft['name'] }}</h3>
                    <p class="text-[#5B5843] mb-2 text-sm futura-400">{{ $craft['community'] }}</p>
                    <p class="text-[#252525] text-lg futura-700">{{ $craft['price'] }}</p>
                </div>
            </div>
            @endforeach
        </div>
